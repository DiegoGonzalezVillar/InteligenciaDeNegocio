...
function CurvaS() {
  ...
  const data = {
    labels: dataFormateada
      .filter((datos, index) => datos.Clase === 0 && index % 5 === 0)
  ...
  const data2 = {
    labels: dataFormateada
      .filter((datos, index) => datos.Clase === 1 && index % 6 === 0)
      .map((datos) => datos.Fecha),
    datasets: [
      {
        label: "Curva S",
  ...
  if (excelData.length > 0) {
    ...
    porcentajeAfisAcumuladas =
      Math.round((ultimoAfisAcumulado.afis_acumuladas / 8500) * 100 * 10) / 10;
    porcentajeMetaAcumulada =
      Math.round((ultimoAfisAcumulado.meta_acumulada / 8500) * 100 * 10) / 10;
