...
const MyComponent = () => {
  ...
  const txtRetiro = async () => {
    ...
    try {
      const response = await fetch(`${URL}txtRetiro`); // Llama al endpoint /ejecutar-bat en el servidor
      const data = await response.text();
      let mensaje = JSON.parse(data).message;
      setMessage(mensaje); // Actualiza el estado con el mensaje de respuesta del servidor
    } catch (error) {
  ...
  const txtCrecimiento = async () => {
    setMessage("");
    setLoading(true); // Establecer el estado de carga en true antes de la solicitud

    try {
      const response = await fetch(`${URL}txtCrecimiento`); // Llama al endpoint /ejecutar-bat en el servidor
      const data = await response.text();
      let mensaje = JSON.parse(data).message;
      setMessage(mensaje); // Actualiza el estado con el mensaje de respuesta del servidor
    } catch (error) {
